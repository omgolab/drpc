...
export function encodeSayHelloRequestJson(obj: { name: string }): Uint8Array {
  return new TextEncoder().encode(
    JSON.stringify(
      toJson(SayHelloRequestSchema, create(SayHelloRequestSchema, obj)),
    ),
  );
}
 ...
 */
export function decodeSayHelloResponseJson(buf: Uint8Array): {
  message: string;
} {
  return fromJson(
    SayHelloResponseSchema,
 ...
 * @returns A Uint8Array containing the encoded message
 */
export function encodeBidiStreamingEchoRequest(obj: {
  name: string;
}): Uint8Array {
  return toBinary(
    BidiStreamingEchoRequestSchema,
    create(BidiStreamingEchoRequestSchema, obj),
  );
}
 ...
 */
export function encodeBidiStreamingEchoRequestJson(obj: {
  name: string;
}): Uint8Array {
  return new TextEncoder().encode(
    JSON.stringify(
 ...
 */
export function decodeBidiStreamingEchoResponse(buf: Uint8Array): {
  greeting: string;
} {
 ...
 */
export function decodeBidiStreamingEchoResponseJson(buf: Uint8Array): {
  greeting: string;
} {
  return fromJson(
    BidiStreamingEchoResponseSchema,
...
export function getCodecFns(contentType: string) {
  // Proto types
  if (contentType.endsWith("+proto") || contentType === "application/proto") {
    return {
  ...
  // JSON types
  if (contentType.endsWith("+json") || contentType === "application/json") {
    return {

...
export async function createSmartHttpLibp2pTransport(
    ...
    const logger = clientOptions.logger.createChildLogger({ contextName: "HTTP-Transport" });
     ...
     */
    async function getResolvedLibp2pTransportForStream(originalHttpUrl: string): Promise<Transport> {
        ...
        if (!p2pAddr) {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    throw new ConnectError(`Failed to fetch /p2pinfo: ${response.status} ${response.statusText}. Body: ${errorText}`, Code.Unavailable);
                }
                const data = await response.json();
                if (!data.Addrs || !Array.isArray(data.Addrs) || data.Addrs.length === 0) {
                    throw new ConnectError("Invalid /p2pinfo response: missing or empty 'Addrs' array", Code.DataLoss);
                }
                ...
                for (const addrStr of data.Addrs) {
                    if (typeof addrStr === 'string' && addrStr.includes('/p2p/')) {
                        try {
                            ...
                            if (addrStr.startsWith('/ip4/127.0.0.1/') || addrStr.startsWith('/ip6/::1/')) {
                                selectedAddr = addrStr;
                                logger.debug(`[SmartTransport] Preferred local p2p address from /p2pinfo: ${selectedAddr}`);
                                break; // Found a local address, use it
                            }
                        } catch (e) {
                            logger.warn(`[SmartTransport] Invalid multiaddr string in /p2pinfo Addrs array: "${addrStr}"`, e);
                        }
                    }
                }
                ...
                if (!selectedAddr && firstP2PAddr) {
                    ...
                    logger.debug(`[SmartTransport] No local p2p address found, using first available: ${selectedAddr}`);
                }

                if (!selectedAddr) {
                    throw new ConnectError("No suitable p2p multiaddress found in /p2pinfo response's 'Addrs' array", Code.DataLoss);
                }
                p2pAddr = selectedAddr;
                p2pAddrCache.set(originalHttpUrl, p2pAddr);
                logger.debug(`[SmartTransport] Resolved ${originalHttpUrl} to ${p2pAddr} and cached.`);
            } catch (error: any) {
                console.error(`[SmartTransport] Error fetching or parsing /p2pinfo for ${originalHttpUrl}:`, error);
                if (error instanceof ConnectError) throw error;
                throw new ConnectError(`Failed to resolve p2p address for ${originalHttpUrl} via /p2pinfo: ${error.message}`, Code.Unavailable);
            }
        } else {
            logger.debug(`[SmartTransport] Using cached p2p address ${p2pAddr} for ${originalHttpUrl}`);
        }
        ...
        resolvedLibp2pTransportInstance = createLibp2pTransport(libp2pNodeInstanceForThisTransport, ma, clientOptions);
    ...
    const smartTransport: Transport = {
        async unary<I extends DescMessage, O extends DescMessage>(
            ...
            logger.debug(`[SmartTransport] Unary call '${method.name}' via HTTP to ${httpServerAddr}`);
            return httpTransport.unary(method, signal, timeoutMs, header, input, contextValues);
        },
        ...
        async stream<I extends DescMessage, O extends DescMessage>(
            ...
            const isClientSideStreaming = method.methodKind === "client_streaming" || method.methodKind === "bidi_streaming";

            if (isClientSideStreaming) {
                logger.debug(`[SmartTransport] Client/Bidi stream '${method.name}' (kind: ${method.methodKind}) detected for HTTP URL ${httpServerAddr}. Attempting /p2pinfo lookup.`);
                try {
                    const libp2pTransportForStream = await getResolvedLibp2pTransportForStream(httpServerAddr);
                    ...
                    console.error(`[SmartTransport] Failed to switch to libp2p for streaming call '${method.name}':`, error);
                    if (error instanceof ConnectError) throw error;
                ...
                logger.debug(`[SmartTransport] Unary/Server stream '${method.name}' via HTTP to ${httpServerAddr}`);
                return httpTransport.stream(method, signal, timeoutMs, header, input, contextValues);
            }
        }
    };

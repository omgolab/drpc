...
export function prepareInitialHeaderPayload(
    ...
    new DataView(procPathLenBuffer.buffer).setUint32(0, procPathBytes.length, false); // false for big-endian

    // Encode content type
    const contentTypeBytes = new TextEncoder().encode(contentTypeValue);
    const contentTypeLenBuffer = new Uint8Array(1);
    new DataView(contentTypeLenBuffer.buffer).setUint8(0, contentTypeBytes.length);
...
export async function parseUnaryResponse<T>(
    ...
    try {
        ...
        if (responseText.startsWith('{')) {
            try {
                ...
                logger.debug(`[parseUnaryResponse] Failed to parse as JSON: ${jsonErr}`);
            }
        }
    } catch (textErr) {
        logger.debug(`[parseUnaryResponse] Failed to convert to string: ${textErr}`);
    }
    ...
    try {
        ...
        if (parseResult.envelope) {
            logger.debug(`[parseUnaryResponse] Successfully parsed as envelope with flags: ${parseResult.envelope.flags}`);

            // If it's a data envelope, try to parse the payload
            if (parseResult.envelope.flags === Flag.NONE && parseResult.envelope.data.length > 0) {
                try {
                    ...
                    logger.debug(`[parseUnaryResponse] Failed to parse envelope data: ${envDataErr}`);
                }
            }
            // If it's an error envelope, try to parse as JSON error
            else if (parseResult.envelope.data && parseResult.envelope.data.length > 0) {
                try {
                    ...
                    logger.debug(`[parseUnaryResponse] Failed to parse error data in envelope: ${errParseErr}`);
                }
            }
        }
    } catch (envelopeErr) {
        logger.debug(`[parseUnaryResponse] Failed to parse as envelope: ${envelopeErr}`);
    }

    // If all parsing methods failed, return a generic error
    const bufHex = responseData ? toHex(responseData.subarray(0, Math.min(responseData.length, 32))) : '';
    return {
        success: false,

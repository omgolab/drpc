...
describe("dRPC Client Content Type Tests", () => {
    ...
    beforeAll(async () => {
        try {
            ...
            console.log(`Using public node HTTP address for content type tests: ${httpAddr}`);
            ...
            await testClientUnaryRequest(client, "Content Type Test Connectivity Check");
        } catch (error) {
            console.error("Failed to set up content type tests:", error);
            throw new Error(`Failed to initialize environment for content type tests: ${error}`);
        }
    });
    ...
    describe("Unary Content Types", () => {
        ...
        importedUnaryContentTypes.forEach(contentType => {
            ...
            const contentTypeName = Object.entries(require("../client/core/types"))
                .find(([key, val]) => val === contentType)?.[0] || contentType;

            it(`should handle unary request with ${contentTypeName}`, async () => {
    ...
    describe("Streaming Content Types", () => {
        ...
        describe("Server Streaming", () => {
            importedStreamingContentTypes.forEach(contentType => {
                ...
                it(`should handle server streaming with ${contentTypeName}`, async () => {
                    try {
        ...
        describe("Client and Bidirectional Streaming", () => {
            importedStreamingContentTypes.forEach(contentType => {
                ...
                it(`should handle client/bidi streaming with ${contentTypeName}`, async () => {
                    try {
                        // Create client with specific content type
                        const client = await createManagedClient(
                            clientManager,
    ...
    describe("Content Type Matrix Test", () => {
        ...
        it("should work with all combinations of unary and streaming content types", async () => {
            ...
            console.log("Running content type matrix test with selected combinations");

            for (const unary of unaryOptions) {
                for (const streaming of streamingOptions) {
                    const unaryName = Object.entries(require("../client/core/types"))
                        .find(([key, val]) => val === unary)?.[0] || unary;

                    const streamingName = Object.entries(require("../client/core/types"))
                        .find(([key, val]) => val === streaming)?.[0] || streaming;

                    console.log(`Testing combination: Unary=${unaryName}, Streaming=${streamingName}`);

...
func TestDiscoveryNotifee(t *testing.T) {
	...
	// Mock the Network() call to return nil (for testing)
	mockHost.On("Network").Return((network.Network)(nil))
	...
	cfg := &hostCfg{logger: logger}
	notifee := &discoveryNotifee{h: mockHost, cfg: cfg}

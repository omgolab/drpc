...
async function getLibp2pHostInstance(options: DRPCOptions): Promise<any> {
    if (!globalHostRef) {
        let logger = options.logger.createChildLogger({ contextName: "dRPC-Client" });
        ...
        globalHostRef.logger.debug("[getLibp2pHostInstance] Reusing existing libp2p host instance.");
...
async function releaseLibp2pHostInstance(): Promise<void> {
    ...
    globalHostRef.logger.debug(`[releaseLibp2pHostInstance] Released libp2p host instance. Remaining refs: ${globalHostRef.refCount}`);
    ...
    if (globalHostRef.refCount <= 0) {
        ...
        logger.debug("[releaseLibp2pHostInstance] Stopping and cleaning up libp2p host instance.");
        try {
...
export async function NewClient<TService extends DescService>(
    ...
    if (addr.startsWith("http://") || addr.startsWith("https://")) {
        ...
        logger.debug(`[NewClient] HTTP/S address detected: ${addr}. Using SmartHttpLibp2pTransport.`);

        // Create the transport and client
        const transport = await createSmartHttpLibp2pTransport(addr, libp2p, options);

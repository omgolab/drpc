...
export function parseEnvelope(buffer: Uint8Array): Envelope {
    if (buffer.length < 5) {
        throw new Error(`Envelope too short: expected at least 5 bytes, got ${buffer.length}`);
    }
    ...
    if (buffer.length < 5 + dataLength) {
        throw new Error(`Envelope too short: expected ${5 + dataLength} bytes, got ${buffer.length}`);
    }

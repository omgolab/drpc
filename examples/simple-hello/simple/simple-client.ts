...
async function main() {
    ...
    if (!peerAddrStr) {
        ...
        console.log('Example: bun ts-hello-client.ts /ip4/127.0.0.1/tcp/9000/p2p/QmHash')
    ...
    if (peerAddrStr) {
        try {
            ...
            if (peerAddrStr.startsWith('/127.0.0.1') || peerAddrStr.startsWith('/localhost')) {
            ...
            for (let i = 0; i < MESSAGES_TO_SEND.length; i++) {
                ...
                try {
                    ...
                    if (err.code !== 'ERR_STREAM_RESET' && !String(err).includes('aborted')) {
                        console.error(`❌ Error reading response for message ${i}:`, err)
                    } else {
                        console.log(`⚠️ Stream was reset or closed by the server after message ${i}`)
                    }
                }
                ...
                if (i < MESSAGES_TO_SEND.length - 1) {
                    ...
                    await stream.close().catch(e => console.error('Error closing stream:', e))
            ...
            await stream.close().catch(e => console.error('Error in final stream close:', e))
        } catch (err) {
            console.error('Error connecting to peer:', err)
        }
    } else {    // Otherwise, discover peers via mDNS

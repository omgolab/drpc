...
// Start the Go server
async function startGoServer(): Promise<{
  goProc: ChildProcess;
  multiAddr: string;
}> {
  ...
  const settlePromise = (
    ...
    if (error) {
      console.error(
        `[test script] Settling promise with error: ${error.message}`,
      );
      rejecter(error);
    } else if (successValue) {
      console.log(
        `[test script] Go server ready. Multiaddr: ${successValue.multiAddr}`,
      );
      resolver(successValue);
    } else {
      console.error(
        "[test script] SettlePromise called without error or success value.",
      );
  ...
  return new Promise<{ goProc: ChildProcess; multiAddr: string }>(
    (resolve, reject) => {
      readinessTimeout = setTimeout(() => {
        settlePromise(
          ...
          reject,
          new Error(
            "Timeout: Go server did not provide multiaddr and become ready within 25 seconds.",
          ),
        );
      }, 25000);
      ...
      rl.on("line", (line) => {
        ...
        const expectedReadinessLine =
          "Go server is running and listening for libp2p connections...";

        if (!detectedMultiAddr && trimmedLine.startsWith(multiAddrPrefix)) {
          let addrPart = trimmedLine.substring(multiAddrPrefix.length);
          const readinessIndexInAddrPart = addrPart.indexOf(
            expectedReadinessLine,
          );
        ...
        if (detectedMultiAddr && isServerReady) {
          settlePromise(resolve, reject, undefined, {
            goProc,
            multiAddr: detectedMultiAddr,
          });
        }

        // Error checks
        if (
          trimmedLine.toLowerCase().includes("panic") ||
          ...
          settlePromise(
            resolve,
            reject,
            new Error(`Go server error: ${trimmedLine}`),
          );
        }
      });
      ...
      goProc.once("exit", (code, signal) => {
        if (!isPromiseSettled) {
          settlePromise(
            ...
            reject,
            new Error(
              `Go server exited prematurely with code ${code} and signal ${signal} before becoming ready.`,
            ),
          );
        }
      });
      ...
      goProc.on("error", (err) => {
        settlePromise(
          resolve,
          reject,
          new Error(`Failed to start Go server process: ${err.message}`),
        );
      });
        ...
        if (goProc && !goProc.killed) {
          if (!isPromiseSettled) {
            settlePromise(
              ...
              () => { },
              new Error(
                "Go process cleanup initiated before server readiness settled.",
              ),
            );
          }
          goProc.kill("SIGTERM");
          await new Promise<void>((termResolve) => {
            const killTimeout = setTimeout(() => {
              console.warn(
                "Go process didn't exit cleanly with SIGTERM after 2s, forcing with SIGKILL",
              );
...
describe("Connect RPC over Libp2p (Using Client Library)", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      console.log(
        "[test script] Libp2p node started with PeerID:",
        node.peerId.toString(),
      );

      const { multiAddr: goServerMultiaddrString, goProc } = goServerInfo;
      const goServerPeerIdString = multiaddr(
        goServerMultiaddrString,
      ).getPeerId();
      if (!goServerPeerIdString) {
        throw new Error(
          `[test script] Could not extract PeerId from Go server multiaddr: ${goServerMultiaddrString}`,
        );
      }
      const goServerPeerId = peerIdFromString(
      ...
      if (testSetupInstance) {
        console.log(
          `[test script] Go server PeerID: ${testSetupInstance.goServerPeerId.toString()}`,
        );
        const serverMaInstance = multiaddr(
          testSetupInstance.goServerMultiaddrString,
        );
        if (testSetupInstance.goServerPeerId && serverMaInstance) {
          await testSetupInstance.node.peerStore.patch(
            testSetupInstance.goServerPeerId as any,
            { multiaddrs: [serverMaInstance] },
          );
          console.log(
            `[test script] Added Go server multiaddr to client peer store`,
          );
          ...
          throw new Error(
            "[test script] Failed to obtain Go server PeerId or Multiaddr for peer store.",
          );
        }
      } else {
        throw new Error(
          "[test script] testSetupInstance is null after assignment.",
        );
      }
      ...
      if (
        testSetupInstance &&
        testSetupInstance.goProc &&
        !testSetupInstance.goProc.killed
      ) {
    ...
    if (
      ...
      try {
        ...
        await new Promise<void>((resolve) => {
          const killTimeout = setTimeout(() => {
            console.log(
              "[test script] Process did not exit with SIGINT, trying SIGKILL...",
            );
            try {
              if (
                testSetupInstance &&
              ...
              const pidCmd = spawn("pgrep", ["-f", "bridge-server"], {
                stdio: "pipe",
              });
  ...
  beforeEach(async () => {
    if (
      !testSetupInstance ||
      !testSetupInstance.node ||
      testSetupInstance.node.status !== "started"
    ) {
    ...
    if (
      ...
      node.getConnections(goServerPeerId as any).length === 0
    ) {
  ...
  it.each(unaryContentTypes)(
    "unary: SayHello request-response (%s)",
    async (contentType) => {
      if (!testSetupInstance) throw new Error("Test setup not initialized.");
      const { goServerPeerId } = testSetupInstance;
      const { encodeSayHelloRequest, decodeSayHelloResponse } =
        getCodecFns(contentType);

      try {
        console.log(
          `[test script] Starting unary test for contentType: ${contentType}`,
        );
  ...
  it.each(streamingContentTypes)(
    "bidirectional: BidiStreamingEcho stream (%s)",
    async (contentType) => {
      if (!testSetupInstance) throw new Error("Test setup not initialized.");
      const { goServerPeerId } = testSetupInstance;
      const {
      ...
      try {
        console.log(
          `[test script] Starting bidirectional streaming test for contentType: ${contentType}`,
        );
        ...
        for (let i = 0; i < responses.length; i++) {
          ...
          expect(responses[i].greeting).toContain(
            `Hello, ${requestMessages[i].name}`,
          );
          console.log(
            `[test script] Verified response ${i + 1}: "${responses[i].greeting}"`,
          );
        }

        console.log(
          "[test script] Bidirectional streaming test completed successfully",
        );
      } catch (error) {
        console.error(
          "[test script] Bidirectional streaming test error:",
          error,
        );
        throw error;
      }

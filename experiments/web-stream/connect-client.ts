 ...
 */
const CONNECT_ONLY_UNARY_PROTO_CONTENT_TYPE =
  protoConnect.contentTypeUnaryProto; // "application/proto";
...
export class ConnectClient {
  ...
  private prepareHeaders(method: string, contentType: string): Buffer {
    ...
    const procPathBuffer = Buffer.concat([
      procPathLenBuffer,
      Buffer.from(procPathBytes),
    ]);
  ...
  async unaryCall<TReq, TResp>(
    ...
    try {
      ...
      this.log(
        `Request encoded to ${requestPayload.length} bytes: ${Buffer.from(requestPayload).toString("hex")}`,
      );
      ...
      this.log(
        `Sending unary request to ${this.servicePath}/${method}, payload size: ${requestPayload.length} bytes`,
      );
      ...
      try {
        ...
        this.log(
          `Successfully decoded raw response: ${JSON.stringify(response)}`,
        );
        return response;
      } catch (decodeError) {
        ...
        try {
          ...
          this.log(
            `Response envelope flags: ${parsedEnvelope.flags}, data size: ${parsedEnvelope.data.length} bytes`,
          );

          if (
            parsedEnvelope.flags === Flag.NONE ||
            parsedEnvelope.flags === Flag.END_STREAM
          ) {
            try {
              ...
              try {
                ...
                throw new Error(
                  `Server returned error: ${JSON.stringify(errorData)}`,
                );
              } catch (jsonError) {
                throw new Error(
                  `Failed to decode response: ${envelopeDecodeError}. Raw data: ${toHex(parsedEnvelope.data)}`,
                );
              }
            }
          } else {
            throw new Error(
              `Unexpected response envelope flags: ${parsedEnvelope.flags}`,
            );
          }
        } catch (envelopeError) {
          ...
          if (responseText.startsWith("{") && responseText.includes('"code"')) {
            try {
              ...
              throw new Error(
                `Server returned raw response (not an envelope): ${responseText}`,
              );
            }
          } else {
            throw new Error(
              `Failed to decode response in any format: ${responseText}`,
            );
          }
        }
      }
  ...
  async bidiStreamingCall<TReq, TResp>(
    ...
    try {
      ...
      for (let i = 0; i < requests.length; i++) {
        ...
        this.log(
          `Prepared request ${i + 1}/${requests.length}, payload size: ${requestPayload.length} bytes`,
        );
      }
      ...
      this.log(
        `Sending ${requests.length} requests in ${dataToSend.length} bytes`,
      );
      ...
      this.log(
        `Received response buffer of size: ${responseBuffer.length} bytes`,
      );
      ...
      while (currentOffset < responseBuffer.length) {
        ...
        if (responseBuffer.length < currentOffset + 5) {
          this.log(
            `Remaining buffer too short for an envelope header at offset ${currentOffset}`,
          );
          break;
        }
        ...
        try {
          ...
          this.log(
            `Parsed envelope at offset ${currentOffset}: flags=${envelope.flags}, data size=${envelope.data.length}`,
          );
              ...
              try {
                ...
                if (errorString.startsWith("{")) {
                  try {
                    const errorData = JSON.parse(errorString);
                    this.log(
                      `Error data with END_STREAM: ${JSON.stringify(errorData)}`,
                    );
                    ...
                    if (
                      ...
                      jsonError.message.startsWith("Server returned error:")
                    ) {
                ...
                if (
                  ...
                  err.message.startsWith("Server returned error:")
                ) {
                  throw err; // Re-throw the error we just created
                }
                this.log(
                  `Non-critical error processing END_STREAM data: ${err}`,
                );
              }
            }
          ...
          this.log(
            `Error parsing envelope at offset ${currentOffset}: ${envelopeError}`,
          );
          break;
        }
      }
